...
function inBounds() {
    if(!(snakeCoorX >= 0 && snakeCoorX < SIZE.WIDHT) || !(snakeCoorY >= 0 && snakeCoorY < SIZE.HEIGHT)) {
        if(snakeCoorX < 0) return snakeCoorX = Math.floor(SIZE.WIDHT);
        if(snakeCoorX > SIZE.WIDHT) return snakeCoorX = 0;
        if(snakeCoorY < 0) return snakeCoorY = Math.floor(SIZE.HEIGHT);
        if(snakeCoorY > SIZE.HEIGHT) return snakeCoorY = 0;
    }
}
